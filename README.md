  ...
  <script>
    function showTab(tabId, element = null) {
      ...
      if (element) {
        ...
        // Fallback if not clicked from nav (e.g. init)
        document.querySelectorAll('nav a').forEach(a => a.classList.remove('active-tab'));
        const navLink = document.querySelector(`nav a[onclick*="${tabId}"]`);
      ...
      skyChart = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            legend: { 
              display: true,
              labels: { font: { family: 'Inter', size: 13 }, color: '#334155' }
            }
          },
          scales: {
            x: { 
            ...
            y: {
              beginAtZero: false,
              title: { display: true, text: 'Million tonnes (Mt)', color: '#64748b', font: { family: 'Inter' } },
              ticks: { color: '#64748b', font: { family: 'Inter' } },
              grid: { color: '#f1f5f9', drawBorder: false }
            }
          },
          ...
          onClick: (evt, elements) => {
            ...
            if (points.length) {
              ...
              info.innerHTML = `
                <h4 style="margin-top:0; color: #0f172a;">${year}</h4>
                <p style="margin-bottom:0.5rem;"><strong>~${value} Mt</strong> estimated fuel consumption.</p>
                <p style="margin-bottom:0.5rem;"><em>Context:</em> ${detail}</p>
                <p style="margin-bottom:0; color:#64748b; font-size: 0.9em;">Higher total fuel burn contributes to CO₂ and non-CO₂ effects (e.g., contrails), amplifying climate impact.</p>
              `;
              info.style.display = 'block';
              info.style.animation = 'fadeIn 0.3s ease-out';
            }
          }
        }
      });
